// 4. Parsing di una data HTTP (RFC 1123)
// Obiettivo: leggere una stringa contenente una data in formato HTTP (RFC 1123),
// ad esempio "Sun, 06 Nov 1994 08:49:37 GMT", e scomporla nei suoi componenti:
// giorno della settimana, giorno del mese, mese, anno, ora, minuti e secondi.
// Si richiede di eseguire il parsing manuale utilizzando funzioni standard come
// strtok() per suddividere la stringa in token e sscanf() per estrarre valori numerici.
#include <unistd.h>
#include <netinet/in.h>
#include <netinet/ip.h>  // Header che fornisce definizioni per l'IP. Include funzionalità avanzate.
#include <sys/types.h>
#include <sys/socket.h>
#include <stdio.h>
#include <string.h>

// Funzione per invertire l'ordine dei byte.
// Necessaria per convertire l'ordine dei byte della porta,
// dall'ordine host a quello di rete e viceversa.
...
int main() {
    ...
    // Lettura dell'entity body: il contenuto della risposta dopo gli header.
    // La lettura continua fino a quando non si raggiunge la fine dei dati
    // o il buffer si esaurisce.
    ...
    // Aggiunge un terminatore di stringa alla fine dell'entity body,
    // rendendo il buffer una stringa valida.
    entity_body[x] = 0;

    // Stampa l'entity body e il numero totale di byte letti dal server.
    printf("%s\n", entity_body);
    printf("Tot bytes = %d\n", x);
